...
class RescueAgent(Agent):
    ...
    class RescueBehaviour(CyclicBehaviour):
        async def run(self):
            ...
            if msg and msg.get_metadata("performative") == "request":
                print(f"[RescueAgent {self.agent.jid}] REQUEST received -> {msg.body}")
                ...
                print(f"[RescueAgent {self.agent.jid}] INFORM sent -> {result}")
    ...
    async def setup(self):
        ...
        print("RescueAgent starting...")
        b = self.RescueBehaviour()
        self.add_behaviour(b)
        print(f"RescueAgent {self.jid} behaviour added: {b}")
