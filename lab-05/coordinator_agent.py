...
class CoordinatorAgent(Agent):
    ...
    async def setup(self):
        ...
        self.rescue_agents = [f"rescue{2000+i}@xmpp.jp" for i in range(3)]
        self.rescue_index = 0
    ...
    class DispatchingState(State):
        async def run(self):
            ...
            target = self.agent.rescue_agents[self.agent.rescue_index]
            self.agent.rescue_index = (self.agent.rescue_index + 1) % len(self.agent.rescue_agents)

            msg = Message(to=target)
            msg.set_metadata("performative", "request")
            msg.body = f"Action Required: {self.agent.decision}"
            await self.send(msg)
            print(f"[DISPATCHING] REQUEST sent to {target}!")
